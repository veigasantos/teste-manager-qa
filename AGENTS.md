...
Sempre execute `npm test` e `npm run build` depois de mudanças. Não reintroduza Vite, Rollup, esbuild ou `tsx`: a política WDAC desta máquina exige assinatura empresarial e bloqueia seus binários nativos. Em Windows, se o Prisma retornar `EPERM` ao gerar o client, encerre apenas os processos Node deste projeto e tente novamente. Se a tela web ficar em branco com erro `Cannot find module '@qa/shared'`, o link do workspace em `node_modules/@qa/shared` foi criado como pasta vazia em vez de link para `packages/shared`; rode `npm install` na raiz (com os processos Node do projeto parados) para recriá-lo. Todo texto de interface e mensagens de erro deve ser digitado/revisado em UTF-8 (PT-BR) — evite editores ou pipes que regravem acentos como `Ã©`, `Ã§Ã£o` etc.
